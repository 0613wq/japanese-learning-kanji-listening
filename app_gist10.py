...
async def _gen_one_async(text, voice, rate):
    try:
...
async def _gen_batch_async(tasks, concurrency=12):
    """tasks: list of dicts {key, text, voice, rate}. Returns dict key -> bytes."""
    sem = asyncio.Semaphore(concurrency)

    async def _wrapped(t):
        async with sem:
            data = await _gen_one_async(t['text'], t['voice'], t['rate'])
            return t['key'], data
...
def _gen_batch_in_thread(tasks, timeout=120):
    """在独立线程里跑 asyncio，避免 Streamlit 主循环冲突。"""
    result_box = [{}]
    err_box    = [None]
    ...
    def _worker():
        ...
        asyncio.set_event_loop(loop)
        try:
            result_box[0] = loop.run_until_complete(_gen_batch_async(tasks))
        except Exception as e:
            err_box[0] = e
        finally:
            loop.close()
...
def generate_loop_audio(words, jp_voice, jp_rate, cn_voice, cn_rate, cn_mode):
    ...
    for i in range(0, len(tasks), chunk_size):
        ...
        chunk_result = _gen_batch_in_thread(chunk)
...
<body>
  ...
  <div class="controls" style="margin-top:-4px;">
    <button id="btnTemp" title="加入/移出临时列表（不影响本次播放）">☆ 临时</button>
    <button id="btnCopy" title="把本次变更复制到剪贴板">📋 复制变更</button>
  </div>
  <div id="pendingBox" style="text-align:center; font-size:12px; color:#7f8c9b; margin:6px 0 8px; min-height:34px;">
    <div id="pendingText">本次未标记变更</div>
    <div id="pendingList" style="font-size:11px; color:#95a5a6; margin-top:2px; max-height:40px; overflow-y:auto;"></div>
  </div>
  <div class="toggles">
...
(function() {
  ...
  // 临时列表本地变更（仅 JS 记录，需用户点「📋 复制变更」→ 粘贴到下方文本框应用）
  const pendingAdd    = new Set();
  const pendingRemove = new Set();

  function isEffectivelyTemp(w) {
    const initial = !!w.in_temp;
    if (initial && pendingRemove.has(w.word)) return false;
    if (!initial && pendingAdd.has(w.word))   return true;
    return initial;
  }

  function updateTempUI() {
    const w = currentWord();
    const eff = isEffectivelyTemp(w);
    btnTemp.textContent = eff ? '⭐ 已临时' : '☆ 临时';
    btnTemp.style.color = eff ? '#e67e22' : '';
    btnTemp.style.borderColor = eff ? '#e67e22' : '';
    const total = pendingAdd.size + pendingRemove.size;
    if (total === 0) {
      pendingTextEl.textContent = '本次未标记变更';
      pendingListEl.textContent = '';
    } else {
      pendingTextEl.innerHTML = '已标记 <b>' + total + '</b> 项变更：' +
        '<span style="color:#27ae60">+' + pendingAdd.size + '</span>　' +
        '<span style="color:#c0392b">−' + pendingRemove.size + '</span>' +
        '　（点 📋 复制后到下方文本框粘贴并应用）';
      const parts = [];
      pendingAdd.forEach(function(w)   { parts.push('<span style="color:#27ae60">+' + w + '</span>'); });
      pendingRemove.forEach(function(w){ parts.push('<span style="color:#c0392b">−' + w + '</span>'); });
      pendingListEl.innerHTML = parts.join('　');
    }
  }
  ...
  // 临时列表标记 —— 只影响 JS 侧的 pending 集合，不干扰当前播放
  btnTemp.addEventListener('click', function() {
    const w = currentWord();
    const initial = !!w.in_temp;
    if (initial) {
      if (pendingRemove.has(w.word)) pendingRemove.delete(w.word);
      else                            pendingRemove.add(w.word);
      pendingAdd.delete(w.word);
    } else {
      if (pendingAdd.has(w.word)) pendingAdd.delete(w.word);
      else                         pendingAdd.add(w.word);
      pendingRemove.delete(w.word);
    }
    updateTempUI();
  });
...
def _panel_loop():
    ...
    est_sec = max(4, int(n_audio * 0.5 / 12))   # 粗略估算
    st.caption(
...
def screen_loop_playing():
    ...
    # ── 临时列表变更：粘贴应用 ──
    with st.expander("⭐ 应用临时列表变更（不打断播放）", expanded=False):
        st.caption(
            "在播放器里点 ⭐ 标记词后，点 「📋 复制变更」→ 粘贴到下框 → 点「💾 应用」。"
            "应用会触发页面刷新，播放会中断（这是必要的）。若不打算保存变更，则忽略此区。"
        )
        changes_text = st.text_area(
            "变更 JSON",
            key="_loop_temp_changes",
            height=90,
            placeholder='{"add": ["食べる", "飲む"], "remove": ["きれい"]}',
            label_visibility="collapsed",
        )

        cc1, cc2 = st.columns([1, 3])
        with cc1:
            apply_clicked = st.button("💾 应用", use_container_width=True, type="primary")

        if apply_clicked:
            txt = (changes_text or "").strip()
            if not txt:
                st.warning("⚠️ 请先粘贴变更 JSON")
            else:
                try:
                    data = _json.loads(txt)
                    add_list    = data.get('add', [])    or []
                    remove_list = data.get('remove', []) or []
                    store = st.session_state.store
                    n_add = n_rem = 0
                    for w in add_list:
                        if store.toggle_temp(w, cat, True):
                            n_add += 1
                    for w in remove_list:
                        if store.toggle_temp(w, cat, False):
                            n_rem += 1
                    # 同步刷新 loop_audio 里的 in_temp 状态，避免重进后按钮显示错
                    _add_set = set(add_list)
                    _rem_set = set(remove_list)
                    for d in audio_data:
                        if d['word'] in _add_set:
                            d['in_temp'] = True
                        elif d['word'] in _rem_set:
                            d['in_temp'] = False
                    st.success(f"✅ 已加入 **{n_add}** 词，移出 **{n_rem}** 词到临时列表")
                    if _gist_enabled():
                        do_gist_save()
                except Exception as e:
                    st.error(f"❌ 解析失败：{e}")
